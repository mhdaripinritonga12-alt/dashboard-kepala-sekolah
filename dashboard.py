...
st.divider()
# =========================================================
# 🔥 LOAD DATA CABDIS PER SHEET (CABDIS_1 s/d CABDIS_14)
# =========================================================
@st.cache_data(show_spinner="📂 Memuat data Kepsek per Cabdis (sheet)...")
def load_data_cabdis():
    xls = pd.ExcelFile(DATA_FILE)
    cabdis_sheets = [s for s in xls.sheet_names if "CABANG_DINAS_PENDIDIKAN_WIL" in s.upper()]

    all_data = []
    for sheet in cabdis_sheets:
        df = pd.read_excel(DATA_FILE, sheet_name=sheet)
        df["SHEET_CABDIS"] = sheet
        all_data.append(df)
    ...
    return df_all, cabdis_sheets

df_kepsek_all, cabdis_sheets = load_data_cabdis()

# =========================================================
# 🔥 NORMALISASI NAMA KOLOM (BIAR AMAN)
# =========================================================
df_kepsek_all.columns = df_kepsek_all.columns.astype(str).str.strip()

# =========================================================
# 🔥 BUAT LIST CABDIS TERURUT 1-14
# =========================================================
def urutkan_sheet_cabdis(sheet_list):
    def ambil_angka(text):
        angka = "".join(filter(str.isdigit, str(text)))
        return int(angka) if angka else 999
    return sorted(sheet_list, key=ambil_angka)

cabdis_sheets_sorted = urutkan_sheet_cabdis(cabdis_sheets)

# =========================================================
# 🔥 MAP STATUS REGULATIF
# =========================================================
def map_status_regulatif(status):
    ...
    if "lebih dari 2" in status or ">2" in status or "lebih dari 2 periode" in status:
        return "Lebih dari 2 Periode"
    if "plt" in status:
        return "PLT"
    if "harus diberhentikan" in status or "diberhentikan" in status:
        return "Harus Diberhentikan"

    return "Lainnya"

# =========================================================
# 🔥 LOGIKA BOLEH EDIT (PERIODE 1 TIDAK BOLEH)
# =========================================================
def cek_boleh_diganti(row):
    status_periode = str(row.get("Status Periode", "")).lower()
    keterangan_akhir = str(row.get("Keterangan Akhir", "")).lower()
    sertifikat = str(row.get("Sertifikat BCKS", "")).lower()

    # ❌ PERIODE 1 TIDAK BOLEH
    if "periode 1" in status_periode or "aktif periode 1" in keterangan_akhir:
        return False

    # ✅ SELAIN PERIODE 1 BOLEH
    if "periode 2" in status_periode:
        return True
    if "lebih dari 2" in status_periode:
        return True
    if "plt" in status_periode:
        return True
    if "harus diberhentikan" in keterangan_akhir:
        return True
...
# =========================================================
# 🔥 PILIH CABDIS DARI SHEET
# =========================================================
st.sidebar.subheader("📌 Pilih Cabang Dinas (Sheet)")
selected_sheet = st.sidebar.selectbox("Cabang Dinas", cabdis_sheets_sorted)

df_cab = df_kepsek_all[df_kepsek_all["SHEET_CABDIS"] == selected_sheet].copy()

# =========================================================
# 🔥 SEARCH SEKOLAH DALAM CABDIS TERPILIH
# =========================================================
st.sidebar.subheader("🔎 Cari Sekolah")
search_sekolah = st.sidebar.text_input("Ketik Nama Sekolah")

if search_sekolah:
    df_cab = df_cab[df_cab["Nama Sekolah"].astype(str).str.contains(search_sekolah, case=False, na=False)]

# =========================================================
# 🔥 REKAP STATUS CABDIS TERPILIH
# =========================================================
st.markdown(f"## 🏫 Data Sekolah — {selected_sheet.replace('_', ' ')}")

df_cab["Status Regulatif"] = df_cab["Keterangan Akhir"].astype(str).apply(map_status_regulatif)

rekap = df_cab["Status Regulatif"].value_counts().reindex([
    "Aktif Periode 1",
    "Aktif Periode 2",
...
c1.metric("Periode 1", int(rekap["Aktif Periode 1"]))
c2.metric("Periode 2", int(rekap["Aktif Periode 2"]))
c3.metric("> 2 Periode", int(rekap["Lebih dari 2 Periode"]))
c4.metric("PLT", int(rekap["PLT"]))
c5.metric("Harus Berhenti", int(rekap["Harus Diberhentikan"]))
c6.metric("Lainnya", int(rekap["Lainnya"]))

st.divider()

# =========================================================
# 🔥 TAMPILKAN GRID SEKOLAH
# =========================================================
cols = st.columns(5)
idx = 0
...
for _, row in df_cab.iterrows():
    ...
    nama_kepsek = row.get("NAMA KASEK", row.get("Nama Kepala Sekolah", "-"))
    ...
    with cols[idx % 5]:
        ...
        with st.expander("📄 Lihat Detail Lengkap"):
            ...
            # tampilkan semua kolom seperti surat
            ...
            if is_view_only:
                ...
                if not boleh_diganti:
                    ...
                    if st.button("💾 Simpan Pengganti", key=f"simpan_{nama_sekolah}", use_container_width=True):
                        ...
                        st.success(f"✅ Pengganti tersimpan: {calon}")
                        st.rerun()
