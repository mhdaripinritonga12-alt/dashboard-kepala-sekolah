...
# =========================================================
# FILE PENYIMPANAN CALON PENGGANTI
# =========================================================

FILE_PENGGANTI = "referensi_rotasi_kepsek.xlsx"

def load_perubahan():

    if not os.path.exists(FILE_PENGGANTI):
        return {}

    try:
        df = pd.read_excel(FILE_PENGGANTI)

        if "Sekolah" not in df.columns or "Calon Pengganti" not in df.columns:
            return {}
...
def save_perubahan(data_dict):
    ...
    df.to_excel(FILE_PENGGANTI, index=False)
...
st.divider()
st.markdown("### 📥 Download Referensi Rotasi Kepala Sekolah")

try:

    df_download = pd.DataFrame([
        {"Sekolah": k, "Calon Pengganti": v}
        for k, v in perubahan_kepsek.items()
    ])
    ...
    st.download_button(
        "📥 Download Referensi Rotasi Kepsek",
        excel.getvalue(),
        "referensi_rotasi_kepsek.xlsx"
    )
